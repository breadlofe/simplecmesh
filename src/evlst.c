...
// Optimizing the Calender Queue (Event queue) is important for the speed of large simulations
// 1) x must be greater than the maximum delay the simulation can inject times rbinwidth
// where x is:
// static ACTIVITY *headspace[x]; /* Head array: elements point to bin list heads */
// static ACTIVITY *tailspace[x]; /* Tail array: elements point to bin list tails */
// static int nbins = x;            /* Number of bins used in a calendar queue      */
// 2) Binwidth must be set correctly or insertion into the queue can be significantly slower
// 3) If conditions 1 and 2 are met, autosize can be set to 0 as the queue is fully optimized

// Event list print funtion now works for the calendar queue

//Brian Neel AMD Fall 2013
...
static ACTIVITY *headspace[20]; /* Head array: elements point to bin list heads */
static ACTIVITY *tailspace[20]; /* Tail array: elements point to bin list tails */
static ACTIVITY **binhead;            /* Pointer to head array in headspace           */
static ACTIVITY **bintail;            /* Pointer to tail array in tailspace           */
static double   binwidth = 0.5;       /* Range of values in a bin                     */
static double   rbinwidth = 2;      /* 1/binwidth                                   */
static int      nbins = 20;            /* Number of bins used in a calendar queue      */
...
static int      autosize = 0;         /* TRUE if autosizing is enabled                */
...
void YS__EventListPrint()    /* Lists the contents of the event list                  */
{
   ...
   if (EventListType == CALQUE)   /* Using calendar queue for the event list          */
   {
	  printf("\n            EVENT LIST: Size = %d, Bucket Range = %g\n",cqsize,binwidth);
      for (i = 0; i<nbins; i++) {
         tp = binhead[i];
		 //if(i == lastbin)
			printf("                Bucket %d:\n",i);
         while (tp != NULL) {
			//if(i == lastbin)
				printf("                    Time of activity %s[%d] is %g\n", tp->name, YS__QeId(tp),tp->time);
            tp = (ACTIVITY*)(tp->next);
			counter++;
         }
      }
